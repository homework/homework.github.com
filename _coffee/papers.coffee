...
entry = (e) ->
  authors = e.author.map (a) -> 
    ...
    if (a of _authors) then name = link {}, name, _authors[a]
    span "author", name
  ...
  address = if "address" in e then span "address", "#{e.address}." else ""
  links = if ("pdf" of e) then link {cl:"url pdf"}, "pdf ", e.pdf else ""
...
papers =
  ...
  render: (tgt) ->
    $.when.apply(null, _promises).then =>
      ...
      tool = link {}, _papers.tool.name, _papers.tool.url
